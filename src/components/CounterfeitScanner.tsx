...
import { Scan, ScanLine, XCircle, CheckCircle2, Maximize, Upload } from "lucide-react";
...
export default function CounterfeitScanner() {
  ...
  }, [autoCapture, captureCameraFrame]);
    ...
    <div className="w-full h-full flex flex-col md:flex-row gap-6">
      ...
      <div className="w-full md:w-2/3 flex flex-col gap-4">
        <div className="relative w-full aspect-video bg-black rounded-xl border border-[#333333] overflow-hidden flex items-center justify-center">
          ...
          {hasPermission === null && (
            <div className="text-[#00f3ff] font-mono text-sm flex flex-col items-center gap-2">
              <Scan className="w-8 h-8 opacity-50 animate-pulse" />
        ...
        <div className="grid gap-3 sm:grid-cols-[1fr_auto_auto]">
          ...
          <button type="button" onClick={() => setAutoCapture((prev) => !prev)} disabled={!hasPermission} className="min-h-12 rounded-lg border border-orange-400/50 px-4 font-mono text-sm text-orange-400 hover:bg-orange-400/10 disabled:cursor-not-allowed disabled:opacity-50">{autoCapture ? `${autoCaptureCountdown}s` : "AUTO CAPTURE"}</button>
          <button
            onClick={handleScan}
